...
import { AlertTriangle, CheckCircle, ArrowLeft, Edit, Trash2 } from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      ...
      const analyzeRes = await api.post('/aquariums/analyze', {
        ...
        species: (aqRes.data.species_links || []).map((link: any) => ({
  ...
  const handleEdit = () => {
    navigate('/calculator', {
      state: {
        ...
        species: (aquarium.species_links || []).map((link: any) => ({
          species_id: link.species.id,
          quantity: link.quantity
        }))
      }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <button 
            ...
            className="flex items-center text-gray-600 hover:text-aqua-600 transition-colors"
          >
          ...
          <div className="flex gap-3">
            ...
            >
              <Edit className="w-4 h-4 mr-1.5" /> Edytuj
            </button>
            <button 
          ...
          <div className="p-8 space-y-8">
            ...
            {report.recommended_parameters && (
              <div className="bg-primary/5 rounded-xl border border-primary/20 p-6 mb-8">
                <h3 className="font-bold text-lg text-primary mb-4">Zalecane Parametry</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-primary/10">
                    <span className="block text-sm text-gray-500 mb-1">Temperatura</span>
                    <span className="font-medium text-gray-900">{report.recommended_parameters.temperature}</span>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-primary/10">
                    <span className="block text-sm text-gray-500 mb-1">pH</span>
                    <span className="font-medium text-gray-900">{report.recommended_parameters.ph}</span>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-primary/10">
                    <span className="block text-sm text-gray-500 mb-1">Zasolenie</span>
                    <span className="font-medium text-gray-900">{report.recommended_parameters.salinity}</span>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-primary/10">
                    <span className="block text-sm text-gray-500 mb-1">Pokarm</span>
                    <span className="font-medium text-gray-900">{report.recommended_parameters.food}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Species List */}
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-4">Wybrane gatunki ({(aquarium.species_links || []).length})</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {(aquarium.species_links || []).map((link: any) => (
                  <div key={link.species.id} className="flex items-center p-3 border rounded-lg bg-white">
                    <img src={link.species.image_url} alt={link.species.name} className="w-12 h-12 rounded bg-gray-100 object-cover mr-3" referrerPolicy="no-referrer" />
                    <div>
                      <p className="font-medium text-sm text-gray-900">{link.species.name} <span className="text-xs font-bold text-aqua-600 bg-aqua-50 px-1 rounded">x{link.quantity}</span></p>
                      <p className="text-xs text-gray-500">Bioload: {link.species.bioload}L</p>
                    </div>
                  </div>
                ))}
              </div>
